    ...
    <PopupState variant="popover" popupId="demo-popup-popover"  >
      {(popupState) => (
        <div> {/*className='border-2 border-black'*/}
          <div  {...bindTrigger(popupState)} className={`${color} border-2 border-black p-2`} >
            {task}
          </div>
          <Popover
          ...
          >
            <div className="divide-y divide-slate-200 text-center" onClick={(e) => {
              ...
              <Typography sx={{ p: 1 }} className={task === 'New' ? 'bg-red-500 text-white border-2 border-white':'bg-red-500 text-white'}>New</Typography>
